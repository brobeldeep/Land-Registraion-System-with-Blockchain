...
async function acceptProperty(propertyId) {
  ...
  try {
    let response = await contract.methods.verifyProperty(propertyId).send({ from: accountUsedToLogin });
    console.log(response);
...
async function rejectProperty(propertyId) {
  ...
  try {
    let response = await contract.methods.rejectProperty(propertyId, "Documents are Not clear").send({ from: accountUsedToLogin });
    console.log(response);
